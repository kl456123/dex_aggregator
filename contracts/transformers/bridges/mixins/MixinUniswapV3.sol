...
contract MixinUniswapV3 {
    ...
    function _tradeUniswapV3(IERC20 sellToken, uint256 sellAmount, bytes memory bridgeData)
        internal
        returns (uint256 boughtAmount)
    {
        QuoteFromUniswapV3Params memory params = abi.decode(bridgeData, (QuoteFromUniswapV3Params));
    ...
    {
        require(
            tokenPath.length >= 2 && tokenPath.length == poolFees.length + 1, "UniswapV3Sampler/invalid path lengths"
        );

...
contract FillQuoteTransformer is IERC20Transformer {
    ...
    constructor(address bridgeAdapter_) {
    ...
    function transform(TransformContext calldata context) external override {
        ...
        data.fillAmount =
            Math.min(_normalizeFillAmount(data.fillAmount, takerTokenBalanceRemaining), context.tokenLimits);

        // Fill the order.
        FillOrderResults memory results;
        results = _fillBridgeOrder(data.bridgeOrder, data);

        require(results.takerTokenSoldAmount == data.fillAmount, "order is not fulfilled completly!");
    }

    // Fill a single bridge order.
    function _fillBridgeOrder(IBridgeAdapter.BridgeOrder memory order, TransformData memory data)
        private
        returns (FillOrderResults memory results)
    {
        uint256 takerTokenFillAmount =
            _computeTakerTokenFillAmount(data, order.takerTokenAmount, order.makerTokenAmount, 0);

        (bool success, bytes memory resultData) = address(bridgeAdapter).delegatecall(
            abi.encodeWithSelector(
                IBridgeAdapter.trade.selector, order, data.sellToken, data.buyToken, takerTokenFillAmount
            )
        );
        if (success) {
        ...
        if (data.side == Side.Sell) {
            ...
            if (orderTakerTokenFeeAmount != 0) {
                takerTokenFillAmount =
                    takerTokenFillAmount * orderTakerAmount / (orderTakerAmount + orderTakerTokenFeeAmount);
            }
        } else {
            // Buy
            takerTokenFillAmount = data.fillAmount * orderTakerAmount / orderMakerAmount;
        }

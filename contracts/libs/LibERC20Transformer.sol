...
library LibERC20Transformer {
    ...
    function transformerTransfer(IERC20 token, address payable to, uint256 amount) internal {
        if (isTokenETH(token)) {
            to.transfer(amount);
        } else {
    ...
    function getTokenBalanceOf(IERC20 token, address owner) internal view returns (uint256 tokenBalance) {
        if (isTokenETH(token)) {
